defmodule Rubik.Transforms do
  ...
  defp corner_transformation(cube, {rotation, {corner_list, direction}}) do
    ...
    Enum.reduce(corner_tuples, cube, fn {from, to}, acc_cube ->
      Map.update!(acc_cube, to, fn _val ->
  ...
  defp edge_transformation(cube, {edges, switch}) do
    ...
    |> Enum.reduce(
      cube,
      fn {from, to}, acc_cube ->
        Map.update!(acc_cube, to, fn _val -> may_switch_edge(Map.get(cube, from), switch) end)
      end
    )
  end
  ...
  def qturns(cube, qturn_list) do
    Enum.reduce(qturn_list, cube, fn transformation, cube ->
