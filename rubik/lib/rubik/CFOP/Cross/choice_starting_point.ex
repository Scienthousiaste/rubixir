defmodule Rubik.Solver.Cross.StartingPoint do
  ...
  defp select_biggest_score(rotation, score, {_rotation_current, score_current})
       when score > score_current do
    {rotation, score}
  end

  defp select_biggest_score(_rotation, score, {rotation_current, score_current})
       when score <= score_current do
  ...
  defp score_edge_for_cross(rotation, edge, cube) do
    score_from_correct_position(correct_position?(edge, cube)) *
  ...
  def select_starting_point(cube, face) do
    ...
    {rotation_with_biggest_score, _score} =
      Enum.reduce(
