defmodule Rubik.Solver.F2L do
  ...
  defp find_next_f2l_goal(
    ...
    goal =
      Enum.find(
        f2l_cubie_duos(face),
        fn [corner, edge] ->
          is_not_placed?(cube, corner) or
            is_not_placed?(cube, edge)
        end
      )
  ...
  defp loop_solve_f2l(solver_data_pre, iter, _f2l_complete = false) do
    solver_data =
      find_next_f2l_goal(solver_data_pre)
      |> complete_f2l_goal

    loop_solve_f2l(solver_data, iter - 1, f2l_completed?(solver_data.cube, solver_data.base_face))
  end
